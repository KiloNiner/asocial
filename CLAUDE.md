...
`exportUserData`/`importUserData` cover circles, friends, journal, custom contact types, activity preferences and `user_settings` — **not** `tasks` (regenerated) or account/session/notification data. Restore is replace-all in a transaction; ids are never trusted from the file — every circle/friend/contact type/interaction gets a fresh server-generated id on every restore (even a plain self-restore), with every reference remapped accordingly, so a client can't choose low-entropy primary keys or collide with another account's rows. A reference that resolves to nothing (a friendId/circleId not in the file, a contactTypeId that is neither in the file nor a real built-in) drops its row rather than failing the restore. `sweepUserContactTasks` (also used by the daily job) then regenerates pending suggestions.

`BACKUP_VERSION` is currently **2** (v1 = no `settings`). Older versions stay restorable: add the new field to `backupSchema` as optional/nullable rather than making the union reject the old shape. `backup-schema.ts` also enforces file-level self-consistency (unique ids, unique circle names, unique composite keys) so a bad file is reported as `invalidFile` instead of aborting the transaction on a raw SQLite error; `importBackup` additionally catches anything that still slips through.
...
### Operational logging
Console logging follows a `[tag] message: JSON` convention (`[boot]`, `[cron]`, `[cron:manual]`, `[digest]`, `[scheduler]`, `[tasks]`, `[auth]`, `[health]`) so `docker logs` shows scheduled-job completions (with stats + duration), notification send failures, and login attempts without querying the DB. A job's stats have to make a quiet run distinguishable from a broken one: the digest sends nothing on most days by design, so `DispatchStats` accounts for every user it considered (`quiet` + `settled` + the ones behind `sent`/`failed`). Keep that property when adding a branch that skips a user. Login logging masks the client IP (last IPv4 octet zeroed / IPv6 collapsed to `/64`) before it's ever logged — keep that when touching `src/actions/auth.ts`.
