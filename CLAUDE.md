...
### Auth (`src/lib/auth/`)
Roll-your-own: argon2id passwords, opaque session token in an httpOnly cookie with only `sha256(token)` stored in DB. Invite-only — the first registered user becomes admin (`isBootstrap()`), everyone else needs a one-time invite. `getCurrentUser()` / `requireUser()` / `requireAdmin()` are the entry points; the `(app)` route-group layout guards authenticated pages. Password reset is admin-driven, not email-driven (SMTP is optional in this app): `password-resets.ts` mirrors the invite-token pattern (hashed, single-use, 24h TTL) — an admin mints a link on the invites admin page and relays it out of band; redeeming it at `/reset-password` calls `destroySessionsForUser` so old sessions don't survive the change.
